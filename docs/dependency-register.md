...
Reviewed on 2026-09-15. Exact direct versions are pinned in `package.json`; the complete graph is fixed by `pnpm-lock.yaml`.
...
| @astrojs/react                   |   6.0.4 | MIT     | Hydrates only the local business-case calculator and demo form.                                              | Accepted |
| React / React DOM                |  19.2.8 | MIT     | Local state and accessible interaction for those two bounded islands; no state library.                      | Accepted |
...
- `pnpm licenses list --prod --json` was reviewed on 2026-09-15. The dependency graph reported open licenses; the LGPL libvips binary is a transitive build-time Sharp asset, not shipped as browser code.
- `pnpm audit --prod --audit-level=high` reported no known vulnerabilities on 2026-09-15.
